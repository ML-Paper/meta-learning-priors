...
def get_bayes_densenet_model_class(prm, task_info):
    ...
    class BasicBlock(nn.Module):
        def __init__(self, in_planes, out_planes, prm, dropRate=0.0):
            ...
            self.conv1 = StochasticConv2d(in_planes, out_planes, kernel_size=3, prm=prm, use_bias=False, stride=1, padding=1)
    ...
    class TransitionBlock(nn.Module):
        def __init__(self, in_planes, out_planes, dropRate=0.0):
            ...
            self.bn1 = nn.BatchNorm2d(in_planes)
            self.relu = nn.ReLU(inplace=True)
            self.conv1 = StochasticConv2d(in_planes, out_planes, kernel_size=1, prm=prm, use_bias=False, stride=1, padding=0)
            self.droprate = dropRate
    ...
    class BayesDenseNet(general_model):
        def __init__(self, depth, growth_rate=12,
            ...
            self.conv1 = StochasticConv2d(input_channels, in_planes, kernel_size=3, prm=prm, use_bias=False, stride=1, padding=1)
            # 1st block
            ...
            self.bn1 = nn.BatchNorm2d(in_planes)
            self.relu = nn.ReLU(inplace=True)
            self.fc = StochasticLinear(in_planes, num_classes, prm)
